...
int RunAllTests()
{
	...
	return runner.RunTestsIf(Test::GetTestList(), nullptr, True(), 0);
}
...
bool TestRunner::IsTestInSuite(const Test* const curTest, char const* suiteName)
{
	using namespace std;
	return (suiteName == nullptr) || !strcmp(curTest->m_details.suiteName, suiteName);
}

void TestRunner::RunTest(TestResults* const result, Test* const curTest, int const maxTestTimeInMs)
{
